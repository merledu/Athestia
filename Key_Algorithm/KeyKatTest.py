    ...
    with open(filepath, 'r') as f:
        ...
        for line in f:
            if line.startswith("count"):
                if test_case:
                    kat_tests.append(test_case)
                test_case = {"count": int(line.split('=')[1].strip())}
            elif line.startswith("seed"):
                test_case["seed"] = bytes.fromhex(line.split('=')[1].strip())  # Seed for key generation      # Convert hex to bytes
            elif line.startswith("pk"):
                test_case["pk"] = line.split('=')[1].strip()  # Expected public key
            elif line.startswith("sk"):
                test_case["sk"] = line.split('=')[1].strip() # Expected secret key
        if test_case:
            kat_tests.append(test_case)
    return kat_tests

def test_keygen(KeyGen, kat_tests):
    for test_case in kat_tests:
        count = test_case['count']  # Test case count
        seed = test_case['seed']   # Seed used for key generation
        expected_pk = test_case['pk']  # Known correct public key from KAT
        expected_sk = test_case['sk']  # Known correct secret key from KAT
        
        # Call your key generation function, which takes the seed as input
        generated_pk, generated_sk = KeyGen(seed)

        public_key_hex = generated_pk.hex()  # Assuming public_key is a bytes object and converting to hex

        # Check if the generated public key matches the expected public key
        if public_key_hex == expected_pk:
            result = "PASS"
        else:
            result = "FAIL"
        ...
        print(f"Expected Public Key: {expected_pk}")
        print(f"\nGenerated Public Key: {public_key_hex}")
...
def main():
    # Define the path to the KAT file within KeyT folder
    kat_file_path = os.path.join('KeyKatT', 'kat_MLDSA_87_det_pure.rsp')  # Change this to the file you want to test
    
    # Load the KAT file for MLDSA-87 key generation tests
    kat_tests = load_kat_file(kat_file_path)
    
    # Test your key generation function using the KAT test vectors
    test_keygen(KeyGen, kat_tests)
