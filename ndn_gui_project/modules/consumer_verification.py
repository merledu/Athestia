...
def verify_signatures(canvas, consumer, data_packet):
    ...
    if ca_count > 1 and not is_root_ca:
        ...
        consumer.message_label.setStyleSheet(
            f"background: white; color: {'green' if ca_verify_result else 'red'}; border-radius: 3px;"
        )
        consumer.message_label.show()
        QApplication.processEvents()
        sleep_with_events(1000)
    ...
    consumer.message_label.setStyleSheet(
        f"background: white; color: {'green' if producer_verify_result else 'red'}; border-radius: 3px;"
    )
    consumer.message_label.show()
    QApplication.processEvents()
    sleep_with_events(1000)
    ...
    consumer.message_label.setStyleSheet(
        f"background: white; color: {'green' if data_verify_result else 'red'}; border-radius: 3px;"
    )
    consumer.message_label.show()
    QApplication.processEvents()
    sleep_with_events(1000)
