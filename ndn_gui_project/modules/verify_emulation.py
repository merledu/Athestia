...
project_dir = cwd / "verify_internal"
if not project_dir.exists():
    download_repo2(cwd)   

all = str(project_dir / "verify_internal.srcs" / "sources_1" / "new")
TB_FILE = str(project_dir / "verify_internal.srcs" / "sim_1" / "new" / "tb.sv")
DEverify_FILE = str(Path(all) / "verifyInternal2.sv")
...
# TB_FILE = r"E:\verify_internal\verify_internal.srcs\sim_1\new\tb_verify.sv"
# DEverify_FILE = r"E:\verify_internal\verify_internal.srcs\sources_1\new\verifyInternal2.sv"
# PKG_FILE = r"E:\verify_internal\verify_internal.srcs\sources_1\new\Dilithium_pkg.sv"
# all=r"E:\verify_internal\verify_internal.srcs\sources_1\new"

print("Exists:", os.path.exists(DEverify_FILE))

VCD_FILE = "verify_output.vcd"
TCL_SCRIPT = "run_verify_simulation.tcl"
...
def write_verifytb_sv(sk,msg,rnd,ctx,msg_len,ctx_len,rnd_len):
...
//                                                   + (Dilithium_pkg::d * Dilithium_pkg::k))) - 1;
    ...
    // DUT instantiation
    verify_internal2 #(.msg_len({msg_len}),.ctx_len({ctx_len}),.rnd_len({rnd_len})) uut (
        .sk(39168'h{sk}),
        .clk(clk),
        .rst(rst),
        .n_msg('h{msg}),
...
def create_tcl_script2():
    ...
    deverify_file_tcl = quote_path(DEverify_FILE)
...
set deverify_file [file normalize {deverify_file_tcl}]
...
# Add the testbench and deverify files
add_files $all
add_files $pkg_file
add_files $tb_file
add_files $deverify_file

# Set the top module
set_property top verify [get_filesets sim_1]

# Launch simulation
launch_simulation

# Add all verifyals and dump to VCD
add_wave /verify/*
open_vcd $vcd_file
log_vcd /verify/*
run all
...
def run_vivado_simulation2():
    if not os.path.exists(TB_FILE):
        ...
        return False
    if not os.path.exists(DEverify_FILE):
        print(f"Error: Deverify file not found at {DEverify_FILE}")
        return False
...
#         verifyals = [sig for sig in vcd.verifyals if sig.startswith("/KeyInternal_tb/uut/")]

#         output_values = {}
#         for sig in verifyals:
...
#         print("\n--- Available verifyals in VCD ---")
#         for sig in vcd.verifyals:
#             try:
...
#         target_verifyals = [
#             "/KeyInternal_tb/uut/dut/sk",  # Adjust depending on exact verifyal path
#             "/KeyInternal_tb/uut/dut/pk"
#         ]

#         output_values = {}
#         for sig in vcd.verifyals:
#             for target in target_verifyals:
...
def verify_emu(sk,msg,rnd,ctx,msg_len,ctx_len,rnd_len):
    print("Starting Vivado simulation...")
    
    write_verifytb_sv(sk,msg,rnd,ctx,msg_len,ctx_len,rnd_len)
    ...
    if outputs:
        ...
        print("\nSimulation Output verifyals:")
        verify=outputs.get("verify.uut.verifyencode.sigma[37015:0]")
        return verify
        # for verifyal, value in outputs.items():
        #     # if (verifyal=="KeyInternal_tb.uut.dut.sk[39167:0]" or verifyal =="KeyInternal_tb.uut.pk[20735:0]"):
        #         print(f"{verifyal}: {value}")
    else:
        print("No verifyals parsed from VCD.")
        return None
...
#     verify_emu(sk,msg,rnd,ctx,3577,77,32)