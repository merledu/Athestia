...
class VivadoInterface:
    ...
    def _parse_vcd_output2(self):
        ...
        if not os.path.exists(self.vcd_file):
                ...
                return None
        try:
            logging.info(f"Starting to parse VCD: {self.vcd_file}")
            vcd = vcdvcd.VCDVCD(self.vcd_file, store_tvs=True)
            logging.info("VCD loaded successfully")
            sig = "/sign/sign"  # Updated to top-level path
            if sig in vcd.signals:
                value_bin = vcd[sig].tv[-1][1]
                if value_bin.startswith('b'): value_bin = value_bin[1:]
                elif value_bin in ('x', 'z'): value_bin = '0'
                value_hex = hex(int(value_bin, 2))
                output = {"signature": value_hex, "sig_id": self.id_prefix + '3'}
                logging.info(f"Parsed signature: {value_hex[:20]}...")
                backup_dir = os.path.join(self.work_dir, "backup_vcd")
                os.makedirs(backup_dir, exist_ok=True)
                backup_path = os.path.join(backup_dir, f"sign_output_{int(time.time())}.vcd")
                shutil.move(self.vcd_file, backup_path)
                logging.info(f"VCD moved to {backup_path} for backup")
                return output
            else:
                logging.warning(f"Signal '{sig}' not found in VCD. Available signals: {list(vcd.signals)}")
                return None
        except Exception as e:
        ...
        def run_simulation_thread():
            if not self._run_vivado_simulation2():
                ...
                if not outputs:
                    ...
                    # Use the confirmed path /sign/uut/sign
                    if "signature" in outputs:
                        result[0] = outputs  # Return the full output dict
                    else:
